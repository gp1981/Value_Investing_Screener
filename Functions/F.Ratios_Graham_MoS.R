...
Ratios_Graham_MoS <- function(DF) {
  ...
  DF <- DF %>%
    mutate(
      MoS = Liq.Value.per.Share * outstandingShares - marketCap_LocalFX,
      MoS_Ratio = (Liq.Value.per.Share  * outstandingShares) / marketCap_LocalFX - 1,
      Price_over_Net.Book.Value = marketCap_LocalFX / (Net.Book.Value.per.Share * outstandingShares)
    )
